...
import pandas as pd
LOGGER = get_logger('gcforest.cascade.stacking_cascade_classifier')
...
class CascadeClassifier(object):
    def __init__(self, ca_config):
        ...
        self.stacked_est_configs = self.get_value("stacked_estimators",None,list)
    ...
    @property
    def n_stacked_estimantors(self):
        # the second layer estimators
        return len(self.stacked_est_configs)
    ...
    def _set_estimator_stack(self, li, ei, est):
        if li not in self.estimator2d_stack:
            self.estimator2d_stack[li] = {}
        self.estimator2d_stack[li][ei] = est

    def _get_estimator_stack(self, li, ei):
        return self.estimator2d_stack.get(li, {}).get(ei, None)
    ...
    def _init_stacked_estimators(self, li, ei):
        est_args = self.stacked_est_configs[ei].copy()
        est_name = "layer_{} - estimator_{} - {}_folds".format(li, ei, est_args["n_folds"])
        # n_folds
        LOGGER.info("=====est_name============{}".format(est_name))
        n_folds = int(est_args["n_folds"])
    ...
    def fit_transform(self, X_groups_train, y_train, X_groups_test, y_test, stop_by_test=False, train_config=None, threshold=None):
        ...
        try:
            ...
            X_proba_train = np.zeros((n_trains, n_classes * (self.n_estimators_1+self.n_stacked_estimantors)), dtype=np.float32)
            ...
            while 1:
                ...
                ### stacking layer
                y_train_proba_li_stack = np.zeros((n_trains, n_classes))
                for ei, est_config in enumerate(self.stacked_est_configs):
                    est = self._init_stacked_estimators(layer_id, ei)
                    # fit_trainsform
                    test_sets = [("test", X_cur_test, y_test)] if n_tests > 0 else None
                    ...
                    y_probas = est.fit_transform(tmp_concat, y_train, y_train,
                            ...
                            keep_model_in_mem=train_config.keep_model_in_mem, threshold=threshold)
                    # train
                    X_proba_train[:, (ei +self.n_estimators_1) * n_classes: (ei+self.n_estimators_1) * n_classes + n_classes] = y_probas[0]
                    y_train_proba_li_stack += y_probas[0]

                    if train_config.keep_model_in_mem:
                        self._set_estimator_stack(layer_id, ei, est)

                y_train_proba_li = y_train_proba_li + y_train_proba_li_stack
                y_train_proba_li /= len(self.est_configs) + len(self.stacked_est_configs)
    ...
    def transform(self, X_groups_test):
        ...
        # probability of each cascades's estimators
        X_proba_test = np.zeros((X_test.shape[0], n_classes * (self.n_estimators_1 + self.n_stacked_estimantors)),
                                      dtype=np.float32)
        X_cur_test = None
        for layer_id in range(self.opt_layer_num):
            ...
            ### stacking layer
            for ei, est_config in enumerate(self.stacked_est_configs):
                est = self._get_estimator_stack(layer_id, ei)
                if est is None:
                    raise ValueError("model (li={}, ei={}) not present, maybe you should set keep_model_in_mem to True".format(
                        layer_id, ei))
                tmp_concate = np.hstack((X_proba_test[:, 0:self.n_estimators_1 * n_classes], X_test))
                y_probas = est.predict_proba(tmp_concate)
                X_proba_test[:, (ei+self.n_estimators_1) * n_classes:(ei+self.n_estimators_1) * n_classes + n_classes] = y_probas
    ...
    def predict_proba(self, X):
        ...
        y_proba = y_proba.reshape((y_proba.shape[0], self.n_estimators_1+self.n_stacked_estimantors, self.n_classes))
