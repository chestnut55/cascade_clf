...
def load_data():
    label_path = osp.abspath(osp.join(get_dataset_base(), "cirrhosis", "labels.txt"))
    y = pd.read_csv(label_path, sep='\t', header=None)
    y = y.T.iloc[0]
    label_encoder = LabelEncoder()
    y = label_encoder.fit_transform(y)

    data_path = osp.abspath(osp.join(get_dataset_base(), "cirrhosis", "count_matrix.csv"))
...
def load_data_phy():
    ...
    otu_path = osp.abspath(osp.join(get_dataset_base(), "cirrhosis", "otu.csv"))
    features = np.genfromtxt(otu_path, dtype=np.str_, delimiter=',')

    my_ref = pd.factorize(my_y)[1]
    label_reference_path = osp.abspath(osp.join(get_dataset_base(), "cirrhosis", "label_reference.txt"))
    f = open(label_reference_path, 'w')
    f.write(str(my_ref))
    f.close()

    newick_path = osp.abspath(osp.join(get_dataset_base(), "cirrhosis", "newick.txt"))
    g = Graph()
...
class T2D(ds_base):
    def __init__(self, **kwargs):
        ...
        if self.data_set == "train":
            ...
            raise ValueError("CIRRHOSIS Unsupported data_set: ", self.data_set)
